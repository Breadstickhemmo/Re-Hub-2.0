...
def update_traits():
    if 'username' in session:
        ...
        jobpos = request.form.get('jobpos')  # Получаем значение должности

        conn = get_db_connection()
        conn.execute(
            'UPDATE users SET personal_traits = ?, professional_traits = ?, jobpos = ? WHERE username = ?',
            (personal_traits, professional_traits, jobpos, session['username'])  # Обновляем также jobpos
        )
        conn.commit()
        conn.close()
...
@app.route('/tarot_result', methods=['GET', 'POST'])
def tarot_result():
    if 'user_id' in session:
        user_id = session['user_id']
        
        # Проверка на наличие данных в запросе
        bday = request.form.get('birth_day')  # Используем .get() для безопасного получения
        bmonth = request.form.get('birth_month')
        byear = request.form.get('birth_year')

        # Проверка на наличие данных
        if not bday or not bmonth or not byear:
            flash("Пожалуйста, заполните все поля даты рождения.", "error")
            return redirect(url_for('tarot_result'))  # Перенаправление на ту же страницу или другую
        ...
        try:
            ...
            users = conn.execute('SELECT username, jobpos FROM users WHERE id != ?', (user_id,)).fetchall()
            ...
            # Перенаправление на страницу результатов расклада с передачей card_id и списка пользователей
            return render_template('tarot_result.html', card_id=card_id, get_card_by_id=get_card_by_id, users=users)
...
def check_user():
    if 'user_id' in session:
        selected_username = request.form['username']  # Получаем имя пользователя из формы

        conn = get_db_connection()
        try:
            # Получаем информацию о текущем пользователе и выбранном пользователе
            current_user = conn.execute(
                'SELECT bday, bmonth, byear FROM users WHERE id = ?',
                (session['user_id'],)
            ).fetchone()

            selected_user = conn.execute(
                'SELECT id, bday, bmonth, byear FROM users WHERE username = ?',
                (selected_username,)
            ).fetchone()

            if not current_user:
                flash("Ваши данные не найдены.", "error")
                return redirect(url_for('tarot_result'))

            if not selected_user:
                flash("Выбранный пользователь не найден.", "error")
                return redirect(url_for('tarot_result'))

            # Проверка, что все поля даты рождения заполнены
            if not all([current_user['bday'], current_user['bmonth'], current_user['byear'],
                        selected_user['bday'], selected_user['bmonth'], selected_user['byear']]):
                flash("У одного из пользователей отсутствуют данные о дате рождения.", "error")
                return redirect(url_for('tarot_result'))

            # Извлекаем даты рождения
            day1, month1, year1 = int(current_user['bday']), int(current_user['bmonth']), int(current_user['byear'])
            day2, month2, year2 = int(selected_user['bday']), int(selected_user['bmonth']), int(selected_user['byear'])

            # Расчет значений для каждой даты
            ...
            # Проверяем, получили ли мы данные о картах
            results = {
                'value1': value1,
                'value2': value2,
                'total_sum': total_sum,
